...
typedef int(__stdcall *fn_get_plugin_list)(char *outBuf, int outBufSize, int *outLen);
...
static fn_get_plugin_list s_managedGetPluginList = nullptr;
...
// Returns true when entityId refers to the local (RevHost) client, which is
// never a plugin-controlled player. Player-scoped plugin events must not fire
// for it, otherwise plugins could cancel/mutate host-only behaviour (movement,
// chat, damage, inventory, commands, teleports, ...).
static bool IsHostClient(int entityId)
{
    ...
    {
        if (p && p->entityId == entityId && p->connection && p->connection->isLocal())
            return true;
    }
    return false;
}
...
void Initialize()
{
    // If the .NET runtime is already loaded (server restart), just re-init
    // plugins on the managed side — function pointers are still valid.
    if (s_runtimeLoaded)
    {
        LogInfo("fourkit", "FourKit reinitializing (runtime already loaded)...");
        s_initialized = true;
        s_managedInit();
        return;
    }
    ...
    // Add plugin-dependencies/ to the DLL search path so that P/Invoke calls
    // (e.g. SQLitePCL.raw → e_sqlite3.dll) can find native libraries placed there.
    std::wstring pluginDepsDir = exeDir + L"\\plugin-dependencies";
    SetDllDirectoryW(pluginDepsDir.c_str());
    ...
    struct { const wchar_t *name; void **target; } entries[] = {
        ...
        {L"GetPluginList", (void **)&s_managedGetPluginList},
...
bool FirePlayerInteractEntity(int playerEntityId,
                              ...
                              float targetHealth, float targetMaxHealth, float targetEyeHeight)
{
    if (IsHostClient(playerEntityId))
        return false;
...
int GetPluginList(char *outBuf, int outBufSize, int *outLen)
{
    if (!s_initialized || !s_managedGetPluginList)
    {
        return 0;
    }

    return s_managedGetPluginList(outBuf, outBufSize, outLen);
}

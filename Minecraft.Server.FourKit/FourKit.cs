...
public static class FourKit
{
    ...
    /// <summary>
    /// Resets all static API state (event handlers, players, commands, worlds,
    /// handler mask) for a clean server restart. Called before re-enabling
    /// plugins so they don't stack duplicate handlers.
    /// </summary>
    internal static void Reset()
    {
        ...
        lock (_worldLock)
        {
            _worldsByDimId.Clear();
        }
        lock (_handlerMaskLock)
        {
            _handlerMask = 0;
            NativeBridge.SetHandlerMask?.Invoke(0);
        }
    }
}

...
public static partial class FourKitHost
{
    ...
    public static void Initialize()
    {
        try
        {
            ...
            // If plugins were already loaded (server restart), just re-enable
            // them. Creating new PluginLoadContexts would duplicate every plugin
            // instance because the old non-collectible AssemblyLoadContexts
            // never unload.
            if (s_loader != null)
            {
                ServerLog.Info("fourkit", $"Re-enabling {s_loader.Plugins.Count} existing plugin(s)...");
                FourKit.Reset();
                s_loader.EnableAll();
    ...
    public static void Shutdown()
    {
        try
        {
            ...
            s_loader?.DisableAll();
            // Do NOT null s_loader — keep the plugins alive so they can be
            // re-enabled on server restart without reloading assemblies.
            // The old non-collectible AssemblyLoadContexts cannot be unloaded
            // anyway, so creating new ones would just duplicate every instance.
            s_scheduler = null;

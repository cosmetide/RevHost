...
public static partial class FourKitHost
{
    ...
    [UnmanagedCallersOnly]
    public static int GetPluginList(IntPtr outBuf, int outBufSize, IntPtr outLenPtr)
    {
        try
        {
            var sb = new System.Text.StringBuilder();
            s_loader?.BuildPluginList(sb);
            if (sb.Length == 0)
            {
                if (outLenPtr != IntPtr.Zero)
                    Marshal.WriteInt32(outLenPtr, 0);
            ...
            return (s_loader?.Plugins.Count ?? 0) + (s_loader?.FailedPlugins.Count ?? 0);
        }
        catch (Exception ex)
        {
            ServerLog.Error("fourkit", $"GetPluginList error: {ex}");
            if (outLenPtr != IntPtr.Zero)
                Marshal.WriteInt32(outLenPtr, 0);
            return 0;
        }

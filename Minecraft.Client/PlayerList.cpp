...
PlayerList::PlayerList(MinecraftServer *server)
{
	...
	InitializeCriticalSection(&m_vanishCS);
}
...
PlayerList::~PlayerList()
{
	...
	DeleteCriticalSection(&m_vanishCS);
}
...
{
	// Never disconnect the RevHost client
	if (player && player->connection && player->connection->isLocal())
		return;
...
void PlayerList::drainPendingDisconnects()
{
	...
	while (!dcCopy.empty())
	{
		...
		// Never disconnect the RevHost client
		if (pd.player->connection != nullptr && pd.player->connection->isLocal())
			continue;
		...
		if (!pd.kickMessage.empty())
		{
			...
			ConsoleLog((pd.player->name + L" was kicked: " + pd.kickMessage).c_str());
		}
		else if (!pd.fourKitHandledQuit)
		{
			if (pd.wasKicked)
			{
				...
				ConsoleLog((pd.player->name + L" was kicked from the game.").c_str());
			}
			else
			{
				broadcastAll(std::make_shared<ChatPacket>(pd.player->name, ChatPacket::e_ChatPlayerLeftGame));
				ConsoleLog((pd.player->name + L" left the game.").c_str());
			}
		}
...
bool PlayerList::placeNewPlayer(Connection *connection, shared_ptr<ServerPlayer> player, shared_ptr<LoginPacket> packet)
{
	...
	if(!player->customTextureUrl.empty() && player->customTextureUrl.substr(0,3).compare(L"def") != 0 && !app.IsFileInMemoryTextures(player->customTextureUrl))
    {
        if (server->getConnection()->addPendingTextureRequest(player->customTextureUrl))
        {
            ...
            ConsoleLog((L"Loading custom skin texture for " + player->name + L": " + player->customTextureUrl).c_str());
#endif
	...
	if(!player->customTextureUrl2.empty() && player->customTextureUrl2.substr(0,3).compare(L"def") != 0 && !app.IsFileInMemoryTextures(player->customTextureUrl2))
	{
		...
		{
#ifndef _CONTENT_PACKAGE
			ConsoleLog((L"Loading custom cape texture for " + player->name + L": " + player->customTextureUrl2).c_str());
#endif
...
#endif
	ConsoleLog((player->name + L" joined the game.").c_str());
	...
	if (ServerRuntime::Security::GetSettings().enableStreamCipher)
	{
		BYTE smallId = 0;
...
void PlayerList::add(shared_ptr<ServerPlayer> player)
{
	...
	// Skip broadcasting info for vanished players
	if( player->connection->getNetworkPlayer() && !isVanished(player) )
	{
...

	{
		...
		for (size_t i = 0; i < snapshot.size(); i++)
		{
			...
			{
				// Skip vanished players when sending info to new joiners
				if (!isVanished(op) || (player->connection && player->connection->isLocal()))
				{
...
bool PlayerList::isVanished(shared_ptr<ServerPlayer> player)
{
	if (!player) return false;
	EnterCriticalSection(&m_vanishCS);
	bool result = m_vanishedEntityIds.count(player->entityId) > 0;
	LeaveCriticalSection(&m_vanishCS);
	return result;
}

void PlayerList::setVanished(shared_ptr<ServerPlayer> player, bool vanished)
{
	if (!player) return;
	...
	EnterCriticalSection(&m_vanishCS);
	if (vanished)
		m_vanishedEntityIds.insert(entityId);
	else
		m_vanishedEntityIds.erase(entityId);
	LeaveCriticalSection(&m_vanishCS);

	if (vanished)
	{
		// Vanished player's info won't be broadcast to new joiners (see add())
		// Hide from broadcastAll for PlayerInfoPackets
		wstring vanishMsg = player->getName() + L" is now vanished.";
		ConsoleLog(vanishMsg.c_str());
	}
	else
	{
		// Re-add vanished player to all other clients
		auto snapshot = getPlayersSnapshot();
		for (auto &other : snapshot)
		{
			if (other == player) continue;
			if (!other->connection || !other->connection->getNetworkPlayer()) continue;

			if (player->connection && player->connection->getNetworkPlayer())
			{
				other->connection->send(std::make_shared<PlayerInfoPacket>(player));
			}
		}
		wstring unvanishMsg = player->getName() + L" is no longer vanished.";
		ConsoleLog(unvanishMsg.c_str());
	}
}

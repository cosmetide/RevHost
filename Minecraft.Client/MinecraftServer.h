...
void ConsoleLog(const wchar_t *message, bool showServer = true);
...
public:
	...
	bool executeConsoleCommandNow(const wstring& msg);
...
//    void addTickable(Tickable tickable);	// 4J removed
	...
	static bool exitAfterStop() { return s_bExitAfterStop; }
	static void setExitAfterStop(bool b) { s_bExitAfterStop = b; }
	static void resetFlags() { s_bServerHalted = false; s_bSaveOnExitAnswered = false; s_bExitAfterStop = false; }
...
public:
	// TPS tracking
	int64_t m_tpsWindowStartMs;
	int m_tpsWindowTicks;
	int64_t m_tpsWindowTicksMs;
	double m_tps5s;
	double m_tps1m;
	double m_tps5m;
	double m_tps10m;
	int m_tps5sTicks;
	int64_t m_tps5sStartMs;
	int m_tps1mTicks;
	int64_t m_tps1mStartMs;
	int m_tps5mTicks;
	int64_t m_tps5mStartMs;
	int m_tps10mTicks;
	int64_t m_tps10mStartMs;

#if defined(_WINDOWS64) && defined(MINECRAFT_SERVER_BUILD)
	unsigned int m_autosaveNextTickMs;
	unsigned int m_autosaveIntervalMs;
...
private:
	...
	bool m_savePerformed;
	bool m_deleteWorldOnExit;
	wstring m_saveFolderName;
	bool m_suspending;
...
public:
	...
	void setExitAfterSave(bool v) { m_exitAfterSave = v; }

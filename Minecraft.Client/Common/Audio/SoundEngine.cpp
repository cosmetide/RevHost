...
#ifdef _WINDOWS64
char SoundEngine::m_szSoundPath[]={"resources\\Windows64Media\\Sound\\"};
char SoundEngine::m_szMusicPath[]={"resources\\music\\"};
...
void SoundEngine::play(int iSound, float x, float y, float z, float volume, float pitch)
{
    ...
    char basePath[256];
    sprintf_s(basePath, "resources/Windows64Media/Sound/%s", (char*)szSoundName);
...
void SoundEngine::playUI(int iSound, float volume, float pitch)
{
    ...
    char basePath[256];
    sprintf_s(basePath, "resources/Windows64Media/Sound/%s/%s", soundDir, ConvertSoundPathToName(name));

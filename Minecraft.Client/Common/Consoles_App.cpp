...
#endif
{
	...
	SetGameSettings(iPad,eGameSetting_RenderDistance,2);
	...
	SetGameSettings(iPad,eGameSetting_ViewBob,0);
	...
	SetGameSettings(iPad,eGameSetting_Hints,0);
	...
	// TU 5
	SetGameSettings(iPad,eGameSetting_Clouds,0);
	...
	// TU 7
	SetGameSettings(iPad,eGameSetting_CustomSkinAnim,0);
...
void CMinecraftApp::ActionGameSettings(int iPad,eGameSetting eVal)
{
	...
	case eGameSetting_RenderDistance:
        if (iPad == ProfileManager.GetPrimaryPad())
        {
            ...
            pMinecraft->options->viewDistance = level;
        };
		break;
...
#if defined(_WINDOWS64) && defined(MINECRAFT_SERVER_BUILD)
				{
					// RevHost dedicated server: trigger the same async save as
					// autosave (LCE-Revelations autosave method). No loading
					// screen; the save runs in the background and is drained
					// on the main thread.
...
#elif _WINDOWS64
	mediapath = L"resources\\Common\\Media\\MediaWindows64.arc";
#elif __ORBIS__

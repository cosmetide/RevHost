...
// #ifdef _XBOX
	...
	port = spConfig.serverPort;
	if (!spConfig.serverIp.empty() && spConfig.serverIp != "0.0.0.0")
		bindIp = spConfig.serverIp.c_str();
#endif
	...
	{
#if defined(MINECRAFT_SERVER_BUILD)
		// Refer to `lan-advertise` and `lan-server-name` in `server.properties`
		bool enableLanAdvertising = spConfig.lanAdvertise;
		std::wstring hostNameWide = ServerRuntime::StringUtils::Utf8ToWide(spConfig.lanServerName);
		const wchar_t* hostName = hostNameWide.c_str();
#else

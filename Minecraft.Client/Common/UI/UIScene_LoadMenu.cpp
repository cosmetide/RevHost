...
UIScene_LoadMenu::UIScene_LoadMenu(int iPad, void *initData, UILayer *parentLayer) : IUIScene_StartGame(iPad, parentLayer)
{
	...
	else
	{
		...
		for(unsigned int i = 0; i < texturePacksCount; ++i)
		{
			...
			// Only show the default texture pack
			if(tp->getId() != 0)
				continue;
...
void UIScene_LoadMenu::tick()
{
	...
	if(m_bSaveThumbnailReady)
	{
		...
		if(bHostOptionsRead)
		{
			...
			m_MoreOptionsParams.bHostPrivileges = TRUE; // RevHost: host privileges always on
	...
	if (m_bAutoStartPending && !m_bSaveThumbnailReady)
	{
		if (++m_iAutoStartDelay > 30)
		{
			m_iAutoStartDelay = 0;
			m_bAutoStartPending = false;
...
void UIScene_LoadMenu::LaunchGame(void)
{
	...
	// Skip all creative/host-privilege confirmation dialogs - go straight to loading
	{
		...
		else
		{
			// set the save to load
			PSAVE_DETAILS pSaveDetails=StorageManager.ReturnSavesInfo();
			if(pSaveDetails == nullptr || m_iSaveGameInfoIndex < 0 || m_iSaveGameInfoIndex >= pSaveDetails->iSaveC)
			{
				app.DebugPrintf("LaunchGame: Invalid save details (pSaveDetails=%p, index=%d, count=%d)\n", pSaveDetails, m_iSaveGameInfoIndex, pSaveDetails ? pSaveDetails->iSaveC : -1);
				m_bIgnoreInput = false;
				return;
			}
#ifndef _DURANGO

	...
	if(result==C4JStorage::EMessage_ResultDecline || result==C4JStorage::EMessage_ResultThirdOption) 
	{
		if( result==C4JStorage::EMessage_ResultDecline ) // Save
		{
			...
			else
			{
				...
				MinecraftServer::getInstance()->setExitAfterSave( true );
...
void IUIScene_PauseMenu::_ExitWorld(LPVOID lpParameter)
{
	...
	if (shouldDeleteHardcoreWorld && !hardcoreSaveFolderName.empty())
	{
		...
		swprintf_s(wSavePath, MAX_PATH, L"Worlds\\%s.ms", hardcoreSaveFolderName.c_str());
		app.DebugPrintf("Hardcore mode: Deleting world save '%ls'\n", wSavePath);
		DeleteFileW(wSavePath);

		wchar_t wThumbPath[MAX_PATH] = {};
		swprintf_s(wThumbPath, MAX_PATH, L"Worlds\\thumbnails\\%s-thumbnail.png", hardcoreSaveFolderName.c_str());
		DeleteFileW(wThumbPath);
	}
#endif

...
UIScene_PauseMenu::UIScene_PauseMenu(int iPad, void *initData, UILayer *parentLayer) : UIScene(iPad, parentLayer)
{
	...
	m_buttons[BUTTON_PAUSE_HELPANDOPTIONS].init(L"Options",BUTTON_PAUSE_HELPANDOPTIONS);
#ifdef _WINDOWS64
	m_buttons[BUTTON_PAUSE_VANISH].init(L"Vanish",BUTTON_PAUSE_VANISH);
#else
...
void UIScene_PauseMenu::handlePress(F64 controlId, F64 childId)
{
	...
	case BUTTON_PAUSE_RESUMEGAME:
		...
		break;
#ifdef _WINDOWS64
	case BUTTON_PAUSE_VANISH:
		{
			Minecraft *pMinecraft = Minecraft::GetInstance();
			if(pMinecraft != nullptr && pMinecraft->localplayers[m_iPad] != nullptr)
			{
				auto player = pMinecraft->localplayers[m_iPad];
				MinecraftServer *server = MinecraftServer::getInstance();
				shared_ptr<ServerPlayer> serverPlayer = nullptr;
				if(server != nullptr && server->getPlayers() != nullptr)
				{
					serverPlayer = server->getPlayers()->getPlayer(player->getName());
				}
				if(player->hasInvisiblePrivilege())
				{
					app.SetGameHostOption(eGameHostOption_HostCanBeInvisible, 0);
					player->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_Invisible, 0);
					player->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_Invulnerable, 0);
					player->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_CanFly, 0);
					player->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_CreativeMode, 0);
					...
					if(serverPlayer)
					{
						serverPlayer->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_Invisible, 0);
						serverPlayer->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_Invulnerable, 0);
						serverPlayer->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_CanFly, 0);
						serverPlayer->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_CreativeMode, 0);
						serverPlayer->setInvisible(false);
						serverPlayer->setGameMode(GameType::SURVIVAL);
					}
					if(server != nullptr && server->getPlayers() != nullptr)
					{
						server->getPlayers()->sendMessage(player->getName(), L"Vanish: OFF");
					}
				}
				else
				{
					app.SetGameHostOption(eGameHostOption_HostCanBeInvisible, 1);
					player->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_Invisible, 1);
					player->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_Invulnerable, 1);
					player->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_CanFly, 1);
					player->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_CreativeMode, 1);
					...
					if(serverPlayer)
					{
						serverPlayer->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_Invisible, 1);
						serverPlayer->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_Invulnerable, 1);
						serverPlayer->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_CanFly, 1);
						serverPlayer->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_CreativeMode, 1);
						serverPlayer->setInvisible(true);
						serverPlayer->setGameMode(GameType::CREATIVE);
					}
					if(server != nullptr && server->getPlayers() != nullptr)
					{
						server->getPlayers()->sendMessage(player->getName(), L"Vanish: ON");
					}
				}
			}
		}
...
int UIScene_PauseMenu::ExitGameSaveDialogReturned(void *pParam,int iPad,C4JStorage::EMessageResult result)
{
	...
	// Don't allow leaving the world while a save is in flight; the pending
	// exit would race the background compression/disc write.
	if (ConsoleSaveFileOriginal::hasPendingBackgroundSave())
	{
		ConsoleLog(L"Cannot exit: a save is in progress. Please wait for it to finish.");
		return 0;
	}
#endif

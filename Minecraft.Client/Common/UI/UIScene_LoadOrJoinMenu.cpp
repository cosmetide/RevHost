...
// Rewrites the LevelName inside a .ms container (level.dat) so the world name
// always comes from the save file itself rather than a folder name or sidecar.
static bool Win64_RenameLevelNameInSave(const wstring& filePath, const wstring& newName)
{
    HANDLE hFile = CreateFileW(filePath.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, nullptr);
    ...
    unsigned char *pContainer = rawData.data();
    unsigned int containerSize = fileSize;
    bool wasCompressed = (*(unsigned int*)rawData.data() == 0);

    if (wasCompressed)
    {
        unsigned int decompSize = *(unsigned int*)(rawData.data() + 4);
        if (decompSize == 0 || decompSize > 128 * 1024 * 1024) return false;
        container.resize(decompSize);
        Compression::getCompression()->Decompress(container.data(), &decompSize, rawData.data() + 8, fileSize - 8);
        pContainer = container.data();
        containerSize = decompSize;
    }

    if (containerSize < 12) return false;

    unsigned int headerOffset = *(unsigned int*)pContainer;
    unsigned int numEntries = *(unsigned int*)(pContainer + 4);
    const unsigned int entrySize = sizeof(FileEntrySaveData);

    if (headerOffset > containerSize || numEntries == 0 || numEntries > 10000 ||
        headerOffset + numEntries * entrySize > containerSize)
    {
        return false;
    }

    FileEntrySaveData *table = (FileEntrySaveData*)(pContainer + headerOffset);
    ...
    if (levelDatIndex < 0) return false;

    unsigned int ldOff = table[levelDatIndex].startOffset;
    unsigned int ldLen = table[levelDatIndex].length;
    if (ldOff >= containerSize || ldLen == 0 || ldOff + ldLen > containerSize || ldLen > 4 * 1024 * 1024) return false;

    byteArray ba;
    ba.data = (byte*)(pContainer + ldOff);
    ba.length = ldLen;
    CompoundTag *root = NbtIo::decompress(ba);
    ...
    dataTag->putString(L"LevelName", newName);

    byteArray newLevelDat = NbtIo::compress(root);
    delete root;
    if (newLevelDat.data == nullptr || newLevelDat.length == 0) return false;

    // rebuild the container: file data sequentially from byte 12, file table at the end
    std::vector<FileEntrySaveData> newTable(numEntries);
    std::vector<unsigned char> rebuilt(SAVE_FILE_HEADER_SIZE);

    for (unsigned int i = 0; i < numEntries; i++)
    {
        const bool isLevelDat = (i == (unsigned int)levelDatIndex);
        const unsigned char *srcPtr = isLevelDat ? newLevelDat.data : pContainer + table[i].startOffset;
        const unsigned int srcLen = isLevelDat ? newLevelDat.length : table[i].length;

        if (!isLevelDat && (table[i].startOffset >= containerSize || table[i].startOffset + srcLen > containerSize))
        {
            delete[] newLevelDat.data;
            return false;
        }

        newTable[i] = table[i];
        newTable[i].length = srcLen;
        newTable[i].startOffset = (unsigned int)rebuilt.size();
        rebuilt.insert(rebuilt.end(), srcPtr, srcPtr + srcLen);
    }

    unsigned int newHeaderOffset = (unsigned int)rebuilt.size();
    *(unsigned int*)rebuilt.data() = newHeaderOffset;
    *(unsigned int*)(rebuilt.data() + 4) = numEntries;

    for (unsigned int i = 0; i < numEntries; i++)
    {
        rebuilt.insert(rebuilt.end(), (unsigned char*)&newTable[i], (unsigned char*)&newTable[i] + entrySize);
    }
    ...
    if (wasCompressed)
    {
        ...
        Compression::getCompression()->Compress(nullptr, &compLen, rebuilt.data(), (unsigned int)rebuilt.size());

        unsigned int actualLen = compLen;
        outFile.resize(compLen + 8);
        Compression::getCompression()->Compress(outFile.data() + 8, &actualLen, rebuilt.data(), (unsigned int)rebuilt.size());

        *(unsigned int*)outFile.data() = 0;
        *(unsigned int*)(outFile.data() + 4) = (unsigned int)rebuilt.size();
        outFile.resize(actualLen + 8);
    }
    else
    {
        outFile.swap(rebuilt);
    }

    delete[] newLevelDat.data;

    HANDLE hOut = CreateFileW(filePath.c_str(), GENERIC_WRITE, FILE_SHARE_READ, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (hOut == INVALID_HANDLE_VALUE) return false;

    DWORD bytesWritten = 0;
    BOOL ok = WriteFile(hOut, outFile.data(), (DWORD)outFile.size(), &bytesWritten, nullptr);
    CloseHandle(hOut);
    return ok && bytesWritten == outFile.size();
}
#endif
...
void UIScene_LoadOrJoinMenu::tick()
{
        ...
        if(!m_bSavesDisplayed)
        {
            ...
            if(m_pSaveDetails!=nullptr)
            {
                ...
                for(unsigned int i = 0; i < m_pSaveDetails->iSaveC; ++i)
                {
                    ...
                    wstring filePath = wstring(L"Worlds\\") + wstring(wFilename) + wstring(L".ms");
...
#ifdef _WINDOWS64
                    {
                        ...
                        if (!levelName.empty())
                        {
                            ...
                            // The thumbnail is stored under the level name, so carry it through for LoadSaveDataThumbnail
                            {
                                int n = WideCharToMultiByte(CP_UTF8, 0, levelName.c_str(), -1,
                                    m_pSaveDetails->SaveInfoA[origIdx].UTF8SaveTitle, MAX_DISPLAYNAME_LENGTH - 1, nullptr, nullptr);
                                if (n <= 0) m_pSaveDetails->SaveInfoA[origIdx].UTF8SaveTitle[0] = '\0';
                                m_pSaveDetails->SaveInfoA[origIdx].UTF8SaveTitle[MAX_DISPLAYNAME_LENGTH - 1] = '\0';
                            }
                        }
                        else
                        {
    ...
    // Auto-load if exactly 1 save world exists
    if (m_bSavesDisplayed && m_bAllLoaded && !m_bExitScene && m_saveDetails != nullptr)
    {
        int saveCount = 0;
        for (unsigned int idx = 0; idx < m_pSaveDetails->iSaveC; ++idx)
        {
            if (m_saveDetails[idx].UTF8SaveName[0] != '\0')
                saveCount++;
        }

        static bool s_autoLoaded = false;
        if (saveCount == 1 && !s_autoLoaded)
        {
            s_autoLoaded = true;
            extern bool g_bAutoStartGame;
            g_bAutoStartGame = true;

            for (unsigned int idx = 0; idx < m_pSaveDetails->iSaveC; ++idx)
            {
                if (m_saveDetails[idx].UTF8SaveName[0] != '\0')
                {
                    int btnIdx = idx + m_iDefaultButtonsC;
                    m_buttonListSaves.setCurrentSelection(btnIdx);
                    m_iSaveListIndex = btnIdx;
                    m_bUpdateSaveSize = true;
                    handlePress((F64)eControl_SavesList, (F64)btnIdx);
                    break;
                }
            }
        }
...
void UIScene_LoadOrJoinMenu::AddDefaultButtons()
{
    ...
    for ( LevelGenerationOptions *levelGen : *app.getLevelGenerators() )
    {
		...
		// Skip mash-up pack worlds (Halo, Mass Effect, Skyrim, etc.)
		if(uiTexturePackID >= 1024)
			continue;
    ...
    if (bRes)
    {
        ...
        if(ui16Text[0]!=0)
        {
...
#elif defined(_WINDOWS64)
            {
                ...
                // The world name lives inside the .ms now (level.dat LevelName) -- rewrite it there
                wchar_t wFilename[MAX_SAVEFILENAME_LENGTH] = {};
                mbstowcs(wFilename, pClass->m_saveDetails[listPos].UTF8SaveFilename, MAX_SAVEFILENAME_LENGTH - 1);
                wstring savePath = wstring(L"Worlds\\") + wstring(wFilename) + wstring(L".ms");

                bool bRenamed = Win64_RenameLevelNameInSave(savePath, wstring(wNewName));
                ...
                UIScene_LoadOrJoinMenu::RenameSaveDataReturned(pClass, bRenamed);
            }
#endif
        }
        else
...
void UIScene_LoadOrJoinMenu::LoadSaveFromCloud()
{
	...
	// RevHost: host privileges (cheats) always stay ON, even if the saved world had them off
...
#ifdef _WINDOWS64
            {
                // Use m_saveDetails (sorted display order) so the correct save slot is targeted
                int displayIdx = pClass->m_iSaveListIndex - pClass->m_iDefaultButtonsC;
                if (pClass->m_pSaveDetails && pClass->m_saveDetails && displayIdx >= 0 &&
                    pClass->m_saveDetails[displayIdx].saveId >= 0 &&
                    pClass->m_saveDetails[displayIdx].saveId < pClass->m_pSaveDetails->iSaveC)
                {
                    StorageManager.DeleteSaveData(&pClass->m_pSaveDetails->SaveInfoA[pClass->m_saveDetails[displayIdx].saveId],
                        UIScene_LoadOrJoinMenu::DeleteSaveDataReturned, (LPVOID)pClass->GetCallbackUniqueId());
                }
                else
                {
                    UIScene_LoadOrJoinMenu::DeleteSaveDataReturned((LPVOID)pClass->GetCallbackUniqueId(), false);
                }
            }
#else

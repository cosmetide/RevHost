...
{
	app.DebugPrintf("[DBG] destroyBlock(%d,%d,%d) player=%ls creative=%d survival=%d tile=%d\n",
		x, y, z, player->name.c_str(), isCreative() ? 1 : 0, isSurvival() ? 1 : 0, level->getTile(x, y, z));
	...
	if (isCreative())
	{
		app.DebugPrintf("[DBG] destroyBlock: CREATIVE path (no playerDestroy)\n");
	...
	else 
	{
		...
		app.DebugPrintf("[DBG] destroyBlock: SURVIVAL path changed=%d canDestroy=%d tile=%d\n", changed ? 1 : 0, canDestroy ? 1 : 0, t);
		if (item != nullptr)
		{

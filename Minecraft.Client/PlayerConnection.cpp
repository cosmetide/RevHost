...
void PlayerConnection::disconnect(DisconnectPacket::eDisconnectReason reason)
{
	...
	// The RevHost client must never be kicked, banned, or disconnected by plugins
	// or any server-side logic. Only allow normal quit/close reasons.
	if (isLocal() &&
		reason != DisconnectPacket::eDisconnect_Closed &&
...
void PlayerConnection::handleCommand(const wstring& message)
{
	...
	if (cmd == L"list" || cmd == L"players")
	{
		...
		wstring msg = L"Online (" + std::to_wstring(count) + L"): " + playerNames;
		send(std::make_shared<ChatPacket>(msg));
		return;
	}

	if (cmd == L"help")
	{
		send(std::make_shared<ChatPacket>(L"=== RevHost Commands ==="));
		send(std::make_shared<ChatPacket>(L"/tps - Show server TPS"));
		send(std::make_shared<ChatPacket>(L"/list - List online players"));
		return;
	}
...
#if defined(_WINDOWS64) && defined(MINECRAFT_SERVER_BUILD)
			server->settings->setBooleanAndSave(L"fire-spreads", app.GetGameHostOption(eGameHostOption_FireSpreads) > 0);
#endif
			app.SetGameHostOption(eGameHostOption_TNT, app.GetGameHostOption(packet->data,eGameHostOption_TNT));
#if defined(_WINDOWS64) && defined(MINECRAFT_SERVER_BUILD)
			server->settings->setBooleanAndSave(L"tnt", app.GetGameHostOption(eGameHostOption_TNT) > 0);
#endif
			app.SetGameHostOption(eGameHostOption_MobGriefing, app.GetGameHostOption(packet->data, eGameHostOption_MobGriefing));
#if defined(_WINDOWS64) && defined(MINECRAFT_SERVER_BUILD)
			server->settings->setBooleanAndSave(L"mob-griefing", app.GetGameHostOption(eGameHostOption_MobGriefing) > 0);
#endif
			app.SetGameHostOption(eGameHostOption_KeepInventory, app.GetGameHostOption(packet->data, eGameHostOption_KeepInventory));
#if defined(_WINDOWS64) && defined(MINECRAFT_SERVER_BUILD)
			server->settings->setBooleanAndSave(L"keep-inventory", app.GetGameHostOption(eGameHostOption_KeepInventory) > 0);
#endif
			app.SetGameHostOption(eGameHostOption_DoMobSpawning, app.GetGameHostOption(packet->data, eGameHostOption_DoMobSpawning));
#if defined(_WINDOWS64) && defined(MINECRAFT_SERVER_BUILD)
			server->settings->setBooleanAndSave(L"do-mob-spawning", app.GetGameHostOption(eGameHostOption_DoMobSpawning) > 0);
#endif
			app.SetGameHostOption(eGameHostOption_DoMobLoot, app.GetGameHostOption(packet->data, eGameHostOption_DoMobLoot));
#if defined(_WINDOWS64) && defined(MINECRAFT_SERVER_BUILD)
			server->settings->setBooleanAndSave(L"do-mob-loot", app.GetGameHostOption(eGameHostOption_DoMobLoot) > 0);
#endif
			app.SetGameHostOption(eGameHostOption_DoTileDrops, app.GetGameHostOption(packet->data, eGameHostOption_DoTileDrops));
#if defined(_WINDOWS64) && defined(MINECRAFT_SERVER_BUILD)
			server->settings->setBooleanAndSave(L"do-tile-drops", app.GetGameHostOption(eGameHostOption_DoTileDrops) > 0);
#endif
			app.SetGameHostOption(eGameHostOption_DoDaylightCycle, app.GetGameHostOption(packet->data, eGameHostOption_DoDaylightCycle));
#if defined(_WINDOWS64) && defined(MINECRAFT_SERVER_BUILD)
			server->settings->setBooleanAndSave(L"do-daylight-cycle", app.GetGameHostOption(eGameHostOption_DoDaylightCycle) > 0);
#endif
...
#if defined(_WINDOWS64) && defined(MINECRAFT_SERVER_BUILD)
			server->settings->setBooleanAndSave(L"natural-regeneration", app.GetGameHostOption(eGameHostOption_NaturalRegeneration) > 0);
#endif

...
void Options::toggle(const Options::Option *option, int dir)
{
    ...
    if (option == Option::RENDER_DISTANCE) viewDistance = (viewDistance + dir) & 31;
...
wstring Options::getMessage(const Options::Option *item)
{
	...
	{
		int idx = (viewDistance >= 0 && viewDistance < 4) ? viewDistance : 0;
        return caption + language->getElement(RENDER_DISTANCE_NAMES[idx]);
    }
...
{
	return viewDistance >= 4 && renderClouds;
}

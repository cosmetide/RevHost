...
GameRenderer::GameRenderer(Minecraft *mc)
{
	...
	renderDistance = static_cast<float>(16 * UIScene_SettingsGraphicsMenu::LevelToDistance(mc->options->viewDistance));
	_tick = 0;
	hovered = nullptr;
	thirdDistance = 4;
	thirdDistanceO = 4;
...
void GameRenderer::tick(bool first)		// 4J - add bFirst
{
	...
	float whiteness = 1.0f - mc->options->viewDistance / 31.0f;
...
void GameRenderer::setupClearColor(float a)
{
	...
	float whiteness = (mc->options->viewDistance + 1.0f) / 32.0f;

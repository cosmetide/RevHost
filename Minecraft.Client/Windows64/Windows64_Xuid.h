...
namespace Win64Xuid
{
	...
	{
		// Process-local cache: uid is immutable during runtime and this path is hot.
		...
		// Try reading uid from client.properties
		if (g_CP_Uid[0] != '\0')
		{
			errno = 0;
			char* end = NULL;
			uint64_t raw = _strtoui64(g_CP_Uid, &end, 0);
			if (end != g_CP_Uid && errno == 0)
			{
				PlayerUID parsed = (PlayerUID)raw;
				if (IsPersistedUidValid(parsed))
				{
		...
		// Fallback: try uid.dat (legacy)
		PlayerUID fileXuid = INVALID_XUID;
		if (ReadUid(&fileXuid))
		{
			s_xuid = fileXuid;
			s_cached = true;
			// Migrate to client.properties
			char hexBuf[32] = {};
			_snprintf_s(hexBuf, sizeof(hexBuf), _TRUNCATE, "0x%016llX", (unsigned long long)s_xuid);
			ClientProps::SetValue("uid", hexBuf);
			strncpy_s(g_CP_Uid, sizeof(g_CP_Uid), hexBuf, _TRUNCATE);
			return s_xuid;
		}

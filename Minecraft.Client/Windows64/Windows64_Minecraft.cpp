...
int g_rScreenWidth = 960;
int g_rScreenHeight = 540;
...
// Client properties globals
char g_CP_Username[17] = { 0 };
char g_CP_Uid[32] = { 0 };
bool g_bAutoStartGame = true;

// Server console
#include <conio.h>
#include <io.h>
static CRITICAL_SECTION g_consoleCS;
static volatile bool g_consoleReady = false;
static volatile bool g_consoleRunning = false;
static HANDLE g_hConOut = nullptr;
static volatile bool g_bServerExiting = false;

volatile LONG g_consolePromptVisible = 0;
wchar_t g_consoleInputBuf[256] = {};
int g_consoleInputLen = 0;
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	ShowWindow(g_hWnd, (nCmdShow != SW_HIDE) ? SW_SHOW : nCmdShow);
	UpdateWindow(g_hWnd);
...
static BOOL WINAPI ConsoleCtrlHandler(DWORD ctrlType)
{
	...
	{
		g_bServerExiting = true;
		g_consoleRunning = false;
		InterlockedExchange(&g_consolePromptVisible, 0);
...
static void InitConsole()
{
	...
	g_hConOut = CreateFileW(L"CONOUT$", GENERIC_WRITE, FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, 0, nullptr);

	FILE *fNul = nullptr;
	freopen_s(&fNul, "NUL", "w", stdout);
	freopen_s(&fNul, "NUL", "w", stderr);

	SetConsoleTitleW(L"RevHost Server Console");

	COORD bufferSize = { 120, 500 };
	SetConsoleScreenBufferSize(g_hConOut, bufferSize);

	SMALL_RECT winSize = { 0, 0, 119, 29 };
	SetConsoleWindowInfo(g_hConOut, TRUE, &winSize);

	DWORD mode;
	GetConsoleMode(g_hConOut, &mode);
	SetConsoleMode(g_hConOut, mode | ENABLE_VIRTUAL_TERMINAL_PROCESSING | ENABLE_PROCESSED_OUTPUT | ENABLE_WRAP_AT_EOL_OUTPUT);

	HANDLE hConIn = GetStdHandle(STD_INPUT_HANDLE);
	DWORD inMode = 0;
	if (hConIn && hConIn != INVALID_HANDLE_VALUE && GetConsoleMode(hConIn, &inMode))
		SetConsoleMode(hConIn, inMode & ~ENABLE_QUICK_EDIT_MODE);

	InitializeCriticalSection(&g_consoleCS);
	SetConsoleCtrlHandler(ConsoleCtrlHandler, TRUE);

	DWORD written;
	SetConsoleTextAttribute(g_hConOut, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE | FOREGROUND_INTENSITY);
	WriteConsoleW(g_hConOut, L"RevHost Server Console\r\n", 24, &written, nullptr);
	SetConsoleTextAttribute(g_hConOut, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
	WriteConsoleW(g_hConOut, L"Waiting for world to load...\r\n", 30, &written, nullptr);
}
...
static DWORD WINAPI ConsoleInputThread(LPVOID)
{
	...
	while (!g_consoleReady && g_consoleRunning)
		Sleep(100);

	// Wait for first tick to complete so join messages appear before the prompt
	Sleep(1000);
	...
	while (g_consoleRunning)
	{
		...
		LockConsoleOutput();
		SetConsoleTextAttribute(g_hConOut, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE | FOREGROUND_INTENSITY);
		WriteConsoleW(g_hConOut, L"server> ", 8, &written, nullptr);
		SetConsoleTextAttribute(g_hConOut, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
		g_consoleInputLen = 0;
		g_consoleInputBuf[0] = L'\0';
		InterlockedExchange(&g_consolePromptVisible, 1);
		UnlockConsoleOutput();
		...
		while (g_consoleRunning)
		{
			...
			else if (ch == L'\b' || ch == 127)
			{
				...
				{
					inputLen--;
					inputBuf[inputLen] = L'\0';
					LockConsoleOutput();
					g_consoleInputLen = inputLen;
					g_consoleInputBuf[inputLen] = L'\0';
					WriteConsoleW(g_hConOut, L"\b \b", 3, &written, nullptr);
					UnlockConsoleOutput();
				}
			}
			else if (ch == 3)
			...
			else if (ch != 0 && ch != 0xE0 && inputLen < 255)
			{
				inputBuf[inputLen++] = (wchar_t)ch;
				inputBuf[inputLen] = L'\0';
				LockConsoleOutput();
				g_consoleInputBuf[inputLen - 1] = (wchar_t)ch;
				g_consoleInputBuf[inputLen] = L'\0';
				g_consoleInputLen = inputLen;
				WriteConsoleW(g_hConOut, &inputBuf[inputLen - 1], 1, &written, nullptr);
				UnlockConsoleOutput();
			}
		}
		...
		if (inputLen > 0)
		{
			...
			if (server != nullptr)
			{
				server->executeConsoleCommandNow(wstring(inputBuf));
			}
		}
	}
...
// ---------------------------------------------------------------------------
// Crash handling: log the exception, write a minidump and show a popup with
// the crash cause instead of exiting silently.
// ---------------------------------------------------------------------------
static const wchar_t *ExceptionCodeToString(DWORD code)
{
	switch (code)
	{
	case 0xC0000005: return L"Access violation (read/write to invalid memory)";
	case 0xC000000D: return L"Invalid handle";
	case 0xC000001D: return L"Invalid instruction";
	case 0xC0000025: return L"Non-continuable exception";
	case 0xC000008C: return L"Array bounds exceeded";
	case 0xC000008D: return L"Floating point denormal operand";
	case 0xC0000094: return L"Integer divide by zero";
	case 0xC0000095: return L"Integer overflow";
	case 0xC00000FD: return L"Stack overflow";
	case 0xC0000135: return L"DLL not found";
	case 0xC0000139: return L"Entry point not found";
	case 0xC0000374: return L"Heap corruption";
	case 0xC0000409: return L"Stack buffer overrun / security check failure";
	case 0xE06D7363: return L"C++ exception (possible memory corruption)";
	case 0x80000003: return L"Breakpoint / debug assert";
	default: return L"Unknown exception";
	}
}

static void WriteCrashLog(DWORD code, const wchar_t *cause, void *address)
{
	...
	wcscat_s(path, L"logs\\crashlog.txt");
	...
	if (_wfopen_s(&f, path, L"a") == 0 && f)
	{
		SYSTEMTIME st;
		GetLocalTime(&st);
		fwprintf(f, L"[%04d-%02d-%02d %02d:%02d:%02d] Exception 0x%08X (%ls) at 0x%p",
			(int)st.wYear, (int)st.wMonth, (int)st.wDay, (int)st.wHour, (int)st.wMinute, (int)st.wSecond,
			(unsigned int)code, cause, address);
		fwprintf(f, L"\n");
		fclose(f);
	}
}
...
static ULONGLONG g_serverStartTick = 0;

static void FormatServerRuntime(wchar_t *buf, size_t cch)
{
	ULONGLONG ms = GetTickCount64() - g_serverStartTick;
	DWORD days = (DWORD)(ms / 86400000ULL);
	DWORD hours = (DWORD)((ms / 3600000ULL) % 24);
	DWORD mins = (DWORD)((ms / 60000ULL) % 60);
	DWORD secs = (DWORD)((ms / 1000ULL) % 60);
	if (days > 0)
		swprintf_s(buf, cch, L"%u day(s) %02u:%02u:%02u", days, hours, mins, secs);
	else
		swprintf_s(buf, cch, L"%02u:%02u:%02u", hours, mins, secs);
}

static DWORD WINAPI CrashWorkerThread(LPVOID param)
{
	CrashContext *ctx = static_cast<CrashContext *>(param);

	WriteCrashLog(ctx->code, ExceptionCodeToString(ctx->code), ctx->address);

	wchar_t runtime[64] = {};
	FormatServerRuntime(runtime, _countof(runtime));

	wchar_t message[1024];
	wsprintfW(message,
		L"The dedicated server has crashed.\n\n"
		L"Exception: 0x%08X - %ls\n"
		L"Address:   0x%p\n"
		L"Runtime:   %ls\n\n"
		L"Check logs\\crashlog.txt for details.",
		(unsigned int)ctx->code, ExceptionCodeToString(ctx->code), ctx->address, runtime);
	MessageBoxW(nullptr, message, L"RevHost Server Crash", MB_OK | MB_ICONERROR | MB_SYSTEMMODAL | MB_TOPMOST);
...
{
	// During a graceful shutdown (stop command or console close) exceptions from
	// the teardown path should terminate silently instead of showing a dialog.
	if (g_bServerExiting || MinecraftServer::exitAfterStop() || MinecraftServer::serverHalted())
		return EXCEPTION_EXECUTE_HANDLER;

	CrashContext *ctx = static_cast<CrashContext *>(calloc(1, sizeof(CrashContext)));
	if (ctx)
	{
		ctx->code = ep->ExceptionRecord->ExceptionCode;
		ctx->address = ep->ExceptionRecord->ExceptionAddress;
		HANDLE hThread = CreateThread(nullptr, 0, CrashWorkerThread, ctx, 0, nullptr);
		if (hThread)
		{
	...
	// Last resort: write the log inline (works even if thread creation fails).
	WriteCrashLog(ep->ExceptionRecord->ExceptionCode,
					   ...
					   _In_ int       nCmdShow)
{
	...
	g_serverStartTick = GetTickCount64();

	// Show a crash popup with the cause instead of exiting silently.
	...
	// Allocate server console window
	InitConsole();

	// Declare DPI awareness so GetSystemMetrics returns physical pixels
	SetProcessDPIAware();
	// Use 1280x720 windowed
	//g_rScreenWidth = GetSystemMetrics(SM_CXSCREEN);
	//g_rScreenHeight = GetSystemMetrics(SM_CYSCREEN);

	// Load client properties (username, uid)
	ClientProps::LoadAll(g_CP_Username, sizeof(g_CP_Username),
						 g_CP_Uid, sizeof(g_CP_Uid));

	// Use client properties username
	if (g_CP_Username[0] != '\0')
	{
		strncpy_s(g_Win64Username, sizeof(g_Win64Username), g_CP_Username, _TRUNCATE);
	}
	...
	// Resolve XUID from client.properties (auto-gen if blank/wrong)
	Win64Xuid::ResolvePersistentXuid();

	// If no username, fall back to "Server"
	...
	// Launch server console input thread
	CreateThread(nullptr, 0, (LPTHREAD_START_ROUTINE)ConsoleInputThread, nullptr, 0, nullptr);
	...
	{
		if ((g_bServerExiting || MinecraftServer::exitAfterStop()) && MinecraftServer::getInstance() == nullptr)
		{
			app.ExitGame();
			break;
		}
		...
		const float* clearColor = kClearColorBlack;
		RenderManager.SetClearColour(clearColor);
		RenderManager.StartFrame();
		if (!app.GetGameStarted())
		{
			RenderManager.SetClearColour(kClearColorBlack); // always black
			RenderManager.Clear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		}
#if 0
		...
		// Drain any completed background save on the main/UI thread (mirrors
		// LCE-Revelations' TickCoreSystems). The blocking StorageManager I/O
		// must never run on the low-stack server thread.

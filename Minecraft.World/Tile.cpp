...
void Tile::spawnResources(Level *level, int x, int y, int z, int data, float odds, int playerBonusLevel)
{
	...
	app.DebugPrintf("[DBG] spawnResources(%d,%d,%d) id=%d data=%d count=%d isClientSide=%d doTileDrops=%d\n",
		x, y, z, id, data, count,
		level->isClientSide ? 1 : 0,
		level->getGameRules()->getBoolean(GameRules::RULE_DOTILEDROPS) ? 1 : 0);
...
{
	bool isClient = level->isClientSide;
	bool doTileDrops = level->getGameRules()->getBoolean(GameRules::RULE_DOTILEDROPS);
	app.DebugPrintf("[DBG] popResource(%d,%d,%d) id=%d isClientSide=%d doTileDrops=%d\n",
		x, y, z, id, isClient ? 1 : 0, doTileDrops ? 1 : 0);
	if( isClient || !doTileDrops ) return;
	...
	item->throwTime = 10;
	app.DebugPrintf("[DBG] popResource: spawning ItemEntity id=%d\n", itemInstance->id);

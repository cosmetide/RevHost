...
#ifdef _WINDOWS64
		{
			// name the thumbnail after the world name stored in the .ms (level.dat LevelName)
			wstring wLevelName;
			if(MinecraftServer::getInstance()!= nullptr && MinecraftServer::getInstance()->levels[0]!=nullptr)
			{
				wLevelName = MinecraftServer::getInstance()->levels[0]->getLevelData()->getLevelName();
			}

			char szThumbName[64] = {};
			sprintf_s(szThumbName, sizeof(szThumbName), "%S", wLevelName.c_str());
			StorageManager.SetSaveThumbnailName(szThumbName);
		}
#endif
...
int ConsoleSaveFileOriginal::SaveSaveDataCallback(LPVOID lpParam,bool bRes)
{
...
#ifdef _WINDOWS64
	// 4J Added: After save completes, capture the save slot name for hardcore world deletion
	if (bRes && app.GetCurrentSaveFolderName().empty())
	{
		...
		if (StorageManager.GetSaveUniqueFilename(szSaveFolder) && szSaveFolder[0] != '\0')
		{
			...
			app.DebugPrintf("SaveSaveDataCallback: captured save slot '%s'\n", szSaveFolder);
		}
	}
#endif
...
{
	// NOTE: No busy-wait here. The background compression thread runs detached;
	// when it completes it stashes the result in s_bgResult. This function just
	// consumes the completed result (quick mutex check) and does the blocking
	// StorageManager I/O, so it must only be called from the main thread.
	PBYTE thumbData = nullptr;
	DWORD thumbSize = 0;
	BYTE textMeta[88] = {};
	int textMetaBytes = 0;
...
{
	// True while compression is running OR while a compressed result is
	// waiting for the main thread to flush it to disc.
	if (s_bgSaveActive.load(std::memory_order_acquire))
		return true;
	std::lock_guard<std::mutex> lk(s_bgSaveMutex);
	return s_bgResult.pending;
}
#endif
